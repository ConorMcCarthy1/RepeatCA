...
export const getSeries = () => {
  ...
  export const getMovieImages = ({ queryKey }) => {
   ...
   export const getSeriesReviews = (id) => {
     return fetch(
       `https://api.themoviedb.org/3/series/${id}/reviews?api_key=${process.env.REACT_APP_TMDB_KEY}`
     )
       .then((res) => res.json())
       .then((json) => {
  ...
  export const getSeriesImages = ({ queryKey }) => {
    ...
    const { id } = idPart;
    return fetch(
      `https://api.themoviedb.org/3/series/${id}/images?api_key=${process.env.REACT_APP_TMDB_KEY}`
    ).then( (response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
      }
